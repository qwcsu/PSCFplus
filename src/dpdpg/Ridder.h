...
template <class Phases, typename T = double>
class Ridder
{
...
public:
    ...
    T solve(Phases phases, T x1, T x2);
...
template <class Phases, typename T>
Ridder<Phases, T>::Ridder(int max_itr, T err)
{
...
template <class Phases, typename T>
T
Ridder<Phases, T>::solve(Phases phases, T x1, T x2)
{
    std::cout << "The initial bracket is [" << x1 << ", " << x2 << "]" << std::endl;
