   ...
   */
   ...
   int AmIterator<D>::solve()
   {
      ...
      if (isFlexible_) {
         ...
         // std::cout << "stress = " << system().mixture().stress(0) << "\n";
         now = Timer::now();
      ...
      for (int itr = 1; itr <= maxItr_; ++itr) {
         ...
         if (done) {
            ...
            // {
            //    std::cout<<"coeffs_[" << i << "] = " << coeffs_[i] << "\n";
            //    std::cout<<"vM_[" << i << "] = " << vM_[i] << "\n";
            // }
            // for(int i = 0; i < nHist_; ++i)
            // {
            //    for(int j = 0; j < nHist_; ++j)
            //    {
            //       std::cout<<"invertMatrix_[" << i << "][" << j << "] = "
            //                << invertMatrix_(i,j) << "\n";
            //    }
            // }
            ...
            // {
            //    std::cout << "  w[" << j << "] = " << systemPtr_->wField(0)[j] << "\n";
            // }
            ...
            // for(int j = 0; j < systemPtr_->mesh().size(); ++j)
            // {
            //    std::cout << "  c[" << j << "] = " << systemPtr_->cFieldRGrid(0)[j] << "\n";
            // }
   ...
   void AmIterator<D>::computeDeviation()
   {
      ...
      for (int i = 0; i < systemPtr_->mixture().nMonomer(); ++i) {
         for (int j = 0; j < systemPtr_->mixture().nMonomer(); ++j) {
            ...
            // std::cout << "idemp( " <<  i << ", " << j << ") = " << systemPtr_->interaction().idemp(i, j) << "\n";
         }
      }
      // exit(1);
      // for(int i = 0; i < systemPtr_->basis().nStar(); ++i)
      //       {
      //           std::cout<< "c " << i << ": " << tempDev[0][i] << "\n";
      //       }
      ...
      if (itr == 1) {
         for (int i = 0; i < mixture.nMonomer(); ++i) {
            for (int j = 0; j < systemPtr_->basis().nStar() - 1; ++j) {
               ...
               // std::cout << "m: " << i << "  w[" << j << "] = " << systemPtr_->wField(i)[j] << "\n";
            }
         }
         ...
         for (int j = 0; j < mixture.nMonomer(); ++j) {
            for (int k = 0; k < systemPtr_->basis().nStar() - 1; ++k) {
               ...
               // std::cout << "w : " << wArrays_[j][k] << "  d = " << dArrays_[j][k] << "\n";
            }
         }
         for (int i = 0; i < nHist_; ++i) {
            for (int j = 0; j < mixture.nMonomer(); ++j) {
               for (int k = 0; k < systemPtr_->basis().nStar() - 1; ++k) {
                  ...
                  dArrays_[j][k] += coeffs_[i] * ( devHists_[i+1][j][k] -
                                                   devHists_[0][j][k] );
                  // std::cout << "tempDev : " << omHists_[i+1][j][k+1] - omHists_[0][j][k+1]<< "\n";
                  // std::cout << "w : " << wArrays_[j][k] << "  d = " << dArrays_[j][k] << "\n";
               }
            }
         }
         ...
         // for (int i = 0; i < mixture.nMonomer(); ++i) {
         //    for (int j = 0; j < systemPtr_->basis().nStar() - 1; ++j) {
         //      std::cout <<  "m: " << i << "  w[" << j << "] = " << systemPtr_->wField(i)[j] << std::endl;
         //    }
         // }
         // if (itr == 2) exit(1);
